...
class Connect:
    ...
    def download_all_file(self):
        ...
        try:
            ...
            print("x")
            query_for_same_manu_and_comp="SELECT * FROM attachments WHERE (manufacturer_id, component_id, attachment_filename, approval_type) IN ( SELECT manufacturer_id, component_id, attachment_filename, approval_type FROM attachments WHERE approval_type = 'By part' GROUP BY manufacturer_id, component_id, attachment_filename, approval_type);"
            res_for_same_manu_and_comp=self.cursor.execute(query_for_same_manu_and_comp)
            index_for_manu_comp=self.cursor.column_names.index("notes")
            id_att=self.cursor.column_names.index("id")
            print("query executed")
            print(res_for_same_manu_and_comp)
            for i in res_for_same_manu_and_comp:
                notes_for_manu_comp=i[index_for_manu_comp]
                if notes_for_manu_comp=='':
                    continue
                else:
                    k=notes_for_manu_comp
                    id_of_comp=i[id_att]
                    upq="update attachments set notes=%s where id=%s"  
                    self.cursor.execute(upq,(k,id_of_comp)) 
                    self.conn.commit()                   
    ...
    #         print(self.id_of_attachments)
    #         print(finalpath)
    #         query_to_update = "UPDATE attachments SET notes=%s WHERE id=%s"
    #         self.cursor.execute(query_to_update, (finalpath, id_of_attachments))
    #         print("File renamed as", finalpath)
    #         self.conn.commit()
    #     except Exception as e:
    #         print("An exception occurred:", e)
